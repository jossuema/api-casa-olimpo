...
@router.get("/categoria/{id}", response_model= schemas.PrendaResponse, description="Obtiene todas las prendas de una categoria")
...
@router.get("/marca/{id}", response_model= schemas.PrendaResponse, description="Obtiene todas las prendas de una marca")
...
@router.get("/categoria/{id_categoria}/marca/{id_marca}", response_model= schemas.PrendaResponse, description="Obtiene todas las prendas de una categoria y marca")
def get_prendas_by_categoria_marca(id_categoria: int, id_marca: int, skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    ...
    if not db_prendas:
        raise HTTPException(status_code=404, detail="Categoria y marca no encontradas")
    return db_prendas

@router.get("color/{color}", response_model= schemas.PrendaResponse, description="Obtiene todas las prendas de un color")
